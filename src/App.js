...
const App = () => {
    ...
    useEffect(() => {searchMovies(movieName)},[btnClicked]);
    return(
        <>
            <h1 className="logo">StreamFlix</h1>
            <Carousel />
            <div className="header">
                <h1 className="repeat">Sleep. Eat. StreamFlix. Repeat.</h1>
                <div className="searchbar">
                    <input type="text" placeholder="Search" value={movieName} 
                        onChange={e => setMovieName(e.target.value)} 
                        onKeyDown={event => {setBtnClicked(event.key === 'Enter')}} 
                    />
                    <button className="search-btn" onClick={()=>setBtnClicked(true)}>
            ...
            <section className="container">  
                {movieList[0]!=false ? movieList.map(movie => <MovieCard movie={movie}/> ):<h1 className="not-found">No Movies/Series Found</h1>}                
            </section>
        </>
    );
}
